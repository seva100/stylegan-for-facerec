...
class Coach:
	def __init__(self, opts, prev_train_checkpoint=None):
		...
		self.n_gpus = opts.n_gpus
		if opts.n_gpus > 1:
			self.gpu_ids = list(range(opts.n_gpus))
		else:
			self.gpu_ids = [0]
		self.device = 'cuda:0'
		...
		# -----
		# NOTE adjusting the crop to BUPT format (hardcoded!)
		self.avg_image = self.avg_image[..., 35:35 + 188, 30:30 + 188]
		self.avg_image = F.interpolate(self.avg_image.unsqueeze(0), 112, mode='bilinear')[0]
		# self.avg_image = F.interpolate(self.avg_image.unsqueeze(0), 256, mode='bilinear')[0]
		# -----
	...
	def perform_train_iteration_on_batch(self, x, y):
		...
		for iter in range(self.opts.n_iters_per_batch):
			...
			# -----
			# NOTE adjusting the crop to BUPT format (hardcoded!)
			# y_hat = y_hat[..., 35:35 + 188, 30:30 + 188]
			y_hat = F.interpolate(y_hat, 112, mode='bilinear')
			# y_hat = F.interpolate(y_hat, 256, mode='bilinear')
			# -----
	...
	def calc_loss(self, x, y, y_hat, latent):
		...
		if self.opts.progressive_steps and self.net.module.encoder.progressive_stage.value != 18:
	...
	def compute_adversarial_loss(self, latent, loss_dict):
		...
		dims_to_discriminate = self.get_dims_to_discriminate() if self.is_progressive_training() else \
			list(range(self.net.module.decoder.n_latent))
	...
	def compute_delta_regularization_loss(self, latent, loss_dict):
		total_delta_loss = 0
		deltas_latent_dims = self.net.module.encoder.get_deltas_starting_dimensions()
		first_w = latent[:, 0, :]
		for i in range(1, self.net.module.encoder.progressive_stage.value + 1):
	...
	def __get_save_dict(self):
		if self.n_gpus == 1:
			...
			save_dict = {
				...
				'optimizer': self.optimizer.state_dict(),
				'best_val_loss': self.best_val_loss,
				'latent_avg': self.net.module.latent_avg
			}
		if self.opts.w_discriminator_lambda > 0:
	...
	def get_dims_to_discriminate(self):
		deltas_starting_dimensions = self.net.module.encoder.get_deltas_starting_dimensions()
		return deltas_starting_dimensions[:self.net.module.encoder.progressive_stage.value + 1]
	...
	def check_for_progressive_training_update(self, is_resume_from_ckpt=False):
		for i in range(len(self.opts.progressive_steps)):
			if is_resume_from_ckpt and self.global_step >= self.opts.progressive_steps[i]:  # Case checkpoint
				self.net.module.encoder.set_progressive_stage(ProgressiveStage(i))
	...
	def sample_real_and_fake_latents(self, x):
		...
		real_w = self.net.module.decoder.get_latent(sample_z)
		fake_w = self.net.module.encoder(x)
