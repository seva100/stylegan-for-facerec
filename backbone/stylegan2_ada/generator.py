import torch
import numpy as np
from backbone.stylegan2_ada.utils import activation_funcs, FullyConnectedLayer, clamp_gain, modulated_conv2d, SmoothUpsample, normalize_2nd_moment, identity
...
class Generator(torch.nn.Module):
    ...
    def __init__(self, z_dim, w_dim, w_num_layers, img_resolution, img_channels, synthesis_layer='stylegan2'):
        super().__init__()
        print('Generator Ada successfully loaded!')
        self.z_dim = z_dim
        self.w_dim = w_dim
    ...
    def forward(self, z, truncation_psi=1, truncation_cutoff=None, noise_mode='random', input_is_latent=False, randomize_noise=None, return_latents=False):
        # print('[Generator, forward] z len:', len(z))
        ...
        if not input_is_latent:
            ...
            out_synth = self.synthesis(ws, noise_mode, return_latents=return_latents)    # can be either [img] or [img, latent] returned
            out_synth = out_synth[0]
        else:
            # z = z[0]
            # print('z:', z.shape)
            out_synth = self.synthesis(z, noise_mode, return_latents=return_latents)
            out_synth = out_synth[0]
        ...
        if return_latents:
            return out_synth, z
        return out_synth, None
        # return out_synth[0]
    
    def mean_latent(self, n_latent):
        # latent_in = torch.randn(
        #     n_latent, self.w_dim, device='cuda:0'    # hardocded
        # )
        latent_in = torch.randn(
            n_latent, self.w_dim, device=self.mapping.layers[0].weight.device
        )
        latent = self.mapping(latent_in, truncation_psi=1, truncation_cutoff=None).mean(0, keepdim=True)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, noise_mode='random', return_latents=False, **kwargs):
        ...
        # print('return_latents:', return_latents)
        if return_latents:
            return img, None
        return [img]
