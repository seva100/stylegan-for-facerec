...
class ArcFace(nn.Module):
    ...
    def forward(self, input, label, onehot_vec=None):
        # --------------------------- cos(theta) & phi(theta) ---------------------------
        ...
        sine = torch.sqrt((1.0 - torch.pow(cosine, 2)).clamp(self.eps, 1 - self.eps))
        ...
        one_hot = torch.zeros(cosine.shape[0], cosine.shape[1] + 1)
        if self.device_id != None:
            one_hot = one_hot.cuda(self.device_id[0])
